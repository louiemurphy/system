...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch('https://sssss-pi.vercel.app/api/requests', { mode: 'cors' });
        if (!response.ok) {
  ...
  const handleSave = async () => {
    if (selectedTeamMember) {
      ...
      try {
        const response = await fetch(`https://sssss-pi.vercel.app/api/requests/${selectedRequest._id}`, {
          method: 'PUT',
          headers: {
  ...
  const handleStatusChange = async (requestId, newStatus) => {
    ...
    try {
      ...
      const response = await fetch(`https://sssss-pi.vercel.app/api/requests/${requestId}`, {
        method: 'PUT',
  ...
  const downloadFile = async (fileUrl, fileName) => {
    try {
      const response = await fetch(`https://sssss-pi.vercel.app${fileUrl}`, {
        method: 'GET',
        headers: {
